(function initMidiSelectionStorage(window) {
  ...
  function readMidiSelection() {
    return normalizeMidiSelectionState({
      selectedInputId: storage?.getItem(MIDI_SELECTION_STORAGE_KEYS.id, '') || '',
      selectedInputName: storage?.getItem(MIDI_SELECTION_STORAGE_KEYS.name, '') || ''
    });
  }
  ...
  function writeMidiSelection(midiState = {}) {
    ...
    if (nextMidiState.selectedInputId) {
      storage?.setItem(MIDI_SELECTION_STORAGE_KEYS.id, nextMidiState.selectedInputId);
    } else {
      storage?.removeItem(MIDI_SELECTION_STORAGE_KEYS.id);
    }

    if (nextMidiState.selectedInputName) {
      storage?.setItem(MIDI_SELECTION_STORAGE_KEYS.name, nextMidiState.selectedInputName);
    } else {
