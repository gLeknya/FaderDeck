(function initMidiActions(window) {
  ...
  function selectMidiInput(inputId, inputName = '', meta = {}) {
    const nextMidiState =
      getMidiService()?.selectInput?.(inputId, inputName, {
        source: 'midi-actions',
        ...meta
      }) || null;
  ...
  function disableMidiInputSelection(meta = {}) {
    const disabledOptionValue =
      getMidiService()?.getDisabledOptionValue?.() || '__disabled__';
  ...
  async function learnChannelFaderMapping(channelId, meta = {}) {
    ...
    window.showToast?.(
      'pending',
      window.t?.('midi.moveFader', {
        name: channel.title || channel.appName
      })
    );

    if (
      !midiService.getSelectedInputId?.() ||
      midiService.isDisabledSelection?.()
    ) {
      window.showToast?.('error', window.t?.('midi.selectDeviceFirst'), {
        updatePending: true
      });
      return null;
    }
    ...
    try {
      ...
      if (error?.code === 'midi_unsupported') {
        window.showToast?.('error', window.t?.('midi.unsupported'), {
          updatePending: true
        });
        return null;
      }

      window.showToast?.('error', window.t?.('midi.initFailed'), {
        updatePending: true
      });
      return null;
    }
    ...
    if (!learned) {
      window.showToast?.('error', window.t?.('midi.failedToDetect'), {
        updatePending: true
      });
    ...
    if (conflict) {
      ...
      const confirmed = await confirmBindingConflict(
        window.t?.('midi.conflict', { name: conflictName })
      );

      if (!confirmed) {
        window.showToast?.('warn', window.t?.('midi.bindCancelled'), {
          updatePending: true
        });
    ...
    persistProfile();
    window.showToast?.('success', window.t?.('midi.bindSuccess'), {
      updatePending: true
    });
    return learned;
  }
  ...
  async function learnChannelButtonMapping(channelId, buttonId, meta = {}) {
    ...
    const conflict = midiService.findButtonMappingConflict?.(
      channelId,
      buttonId,
      learned
    );
    ...
    if (conflict) {
      const conflictName =
        ...
        'Control';
      const confirmed = await confirmBindingConflict(
        window.t?.('midi.buttonConflict', { name: conflictName })
      );
    ...
    persistProfile();
    midiService.flashChannelButtonBindingFeedback?.(channelId, buttonId, {
      reason: 'button-bind'
    });
    midiService.syncChannelButtonIndicators?.({ reason: 'button-bind' });
    window.showToast?.('success', window.t?.('midi.buttonBindSuccess'), {
      updatePending: true
    });
    return learned;
  }
  ...
  async function learnStandaloneButtonMapping(buttonId, meta = {}) {
    ...
    persistProfile();
    midiService.flashStandaloneButtonBindingFeedback?.(buttonId, {
      reason: 'button-bind'
    });
    midiService.syncChannelButtonIndicators?.({ reason: 'button-bind' });
    window.showToast?.('success', window.t?.('midi.buttonBindSuccess'), {
